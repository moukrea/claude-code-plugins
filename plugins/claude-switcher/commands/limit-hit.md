...
3. If it fails (auto-switch not enabled or no fallback configured), help them set it up using slash commands:
   - `/auto-config enable`
   - `/auto-config primary <name>`
   - `/auto-config fallback <name>`
